"""
vis_feature_activation.py — CNN特征激活图可视化（全部19个子带）

所有19个子带都叠加CNN特征激活热力图：
  - 有信号子带：激活图应呈现集中/结构化模式
  - 无信号子带：激活图应呈现均匀/随机分布

用法:
  python vis_feature_activation.py B M10 --sample 2
  python vis_feature_activation.py B M10 --sample 2 --layer deep
python vis_feature_activation.py B M10 --sample 2 --layer mid
python vis_feature_activation.py B M10 --sample 2 --layer shallow
"""
...
def main():
    ...
    with h5py.File(test_path, 'r') as f:
        ...
        if src_pos.shape[0] != len(src_count):
            src_pos = src_pos.transpose(2, 1, 0)

    N_sub = cfg['N_sub']
    sample_idx = args.sample
    n_src = int(src_count[sample_idx])
    print(f"\nSample #{sample_idx}: {n_src} source(s)")

    # ── 预处理 ──
    spec = spectra_raw[sample_idx]
    ...
    with torch.no_grad():
        ...
        band_pred = (probs > args.threshold).long()

    pred_count = (band_pred[0].sum(dim=-1) > 0).sum().item()
    print(f"Predicted: {pred_count} source(s)")
    band_pred_np = band_pred[0].cpu().numpy()

    # ── 提取特征激活图 ──
    extractor = MultiLayerExtractor(model)
    activation = extractor.get_activation(model, input_tensor, args.layer)
    layer_info = {'shallow': '(32ch, 41x41)', 'mid': '(64ch, 21x21)', 'deep': '(128ch, 11x11)'}
    print(f"Layer: {args.layer} {layer_info[args.layer]}")
    ...
    for k in range(N_sub):
        ...
        has_signal = any(band_pred_np[s, k] == 1 for s in range(cfg['max_src']))
        flag = '*' if has_signal else ' '
        print(f"  {flag} W{k+1:2d}: mean={act.mean():.3f}  max={act.max():.3f}  std={act.std():.3f}")
    ...
    true_pos = [src_pos[sample_idx, s, :] for s in range(n_src)]
    ...
    # colorbar用不透明版本（否则colorbar也透明看不清）
    cdict_solid = {
        'red':   [(0.0, 0.0, 0.0), (0.3, 0.0, 0.0), (0.5, 0.0, 0.0),
                  (0.7, 1.0, 1.0), (1.0, 0.85, 0.85)],
        'green': [(0.0, 0.0, 0.0), (0.3, 0.2, 0.2), (0.5, 0.6, 0.6),
    ...
    cmap_solid = LinearSegmentedColormap('act_solid', cdict_solid, N=256)
    ...
    fig.suptitle(f'Sample #{sample_idx}   Truth: {n_src} src   Pred: {pred_count} src\n'
                 f'CNN {args.layer} layer {layer_info[args.layer]} feature activation',
